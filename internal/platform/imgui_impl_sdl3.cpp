...
#endif
    // xerotty patch: enable multi-viewport unconditionally. Upstream
    // gates this on MouseCanUseGlobalState (whitelist: windows/cocoa/x11)
    // — Wayland is excluded because global mouse tracking isn't
    // available there (compositor security), which kills the
    // "drag title bar out to detach" gesture. xerotty doesn't use
    // that gesture though — it pops viewports out programmatically
    // for prefs windows and "new window" commands, which work fine
    // without global mouse. Without this patch every popup ends up
    // rendered inside the main viewport instead of as its own OS
    // window. HasParentViewport stays gated as upstream has it.
...
static void ImGui_ImplSDL3_UpdateMouseData()
{
    ...
    if (is_app_focused)
    {
        ...
        // from SDL_GetWindowPosition, while SDL_GetGlobalMouseState is
        // ABSOLUTE — the fallback's global - position math is wrong by
        // the parent's origin for them. Upstream now also skips the
        // fallback whenever SOME window is hovered, but macOS popups'
        // tracking areas don't reliably register the cursor (hovered
        // reads null over a popup), so keep the explicit popup guard.
        ...
        if (hovered_window == nullptr && bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0 && !is_relative_mouse_mode && !focused_is_popup)
        {
...
#endif
    // xerotty patch: SDL3 turns text input OFF per-window and only
    // the parent gets SDL_StartTextInput'd by the app. Without
    // enabling it on each popped-out viewport too, the popout
    // window accepts key events (Enter, arrows) but silently drops
    // all character input — terminals can't be typed into.
    SDL_StartTextInput(vd->Window);
